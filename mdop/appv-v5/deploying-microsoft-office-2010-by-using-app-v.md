...
Пакеты Office 2010 для Application 5,0 Virtualization можно создать с помощью одного из указанных ниже способов.
...
## Создание Office 2010 App-V 5,0 с помощью секвенсора


Виртуализация Office 2010 — это один из основных методов создания пакета Office 2010 на App-V 5,0. Корпорация Майкрософт предоставила подробный рецепт в статье базы знаний. Чтобы создать пакет Office 2010 для App-V 5,0, ознакомьтесь с подробными инструкциями по следующей ссылке:
...
## Создание пакетов Office 2010 App-V 5,0 с помощью ускорителей пакетов


Пакеты Office 2010 App-V 5,0 можно создавать с помощью ускорителей пакетов. Корпорация Майкрософт предоставила ускорители пакетов для создания Office 2010 в Windows 8 и Windows 7. Чтобы создать пакеты Office 2010 в App-V с помощью ускорителей пакетов, просмотрите следующие страницы, чтобы получить доступ к соответствующему ускорительу пакетов:
...
Подробные инструкции по созданию пакетов виртуальных приложений с помощью ускорителей пакетов App-V можно найти в разделе [Создание виртуального пакета приложения с помощью ускорителя пакетов App-v](how-to-create-a-virtual-application-package-using-an-app-v-package-accelerator.md).

## Развертывание пакета Microsoft Office для App-V 5,0
...
Пакеты Office 2010 могут управляться так же, как и любые другие пакеты App-V 5,0 с помощью стандартных механизмов управления пакетами. Специальные инструкции не требуются, например, для добавления, публикации, отмены публикации и удаления пакетов Office.
