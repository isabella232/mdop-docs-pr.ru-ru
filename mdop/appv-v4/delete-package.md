...
Удаляет запись пакета и связанные с ней приложения.
...
<td align="left"><p>Имя удаляемого пакета.</p></td>
</tr>
<tr class="even">
...
**Важно!**  Команда удалить пакет всегда выполняет глобальное удаление пакета и удаляет только глобальные типы файлов и сочетания клавиш.

Если пакет является глобальным, эту команду необходимо запустить от имени локального администратора; в противном случае требуется только разрешение **DeleteApp** .
